    ...
    success: function (result) {
      ...
      function drawChart() {
        ...
        var options = {
          title: "",
          is3D: true,
          backgroundColor: isDarkMode ? "#343a40" : "#ffffff", // Dark mode background
          legendTextStyle: { color: isDarkMode ? "#ffffff" : "#000000" }, // Legend color
          titleTextStyle: { color: isDarkMode ? "#ffffff" : "#000000" }, // Title color
          pieSliceTextStyle: { color: isDarkMode ? "#ffffff" : "#000000" }, // Slice text color
        };
      ...
      // ✅ Optional: Re-draw chart on dark mode toggle (if you have a toggle button)
...
function status_date_wise_search_sales() {
  ...
  $(".cancel_order_report_table").bootstrapTable("refresh");
}
...
function category_query_params(p) {
  return {
    ...
    status: $("#status").val(),
    commission_method: $("#commission_method").val(),
    limit: p.limit,
    sort: p.sort,
    order: p.order,
    offset: p.offset,
...
function orders_query_params(p) {
  return {
    ...
    is_self_pick_up: $("#is_self_pick_up").val(),
    limit: p.limit,
    sort: p.sort,
    order: p.order,
    offset: p.offset,
...
function category_sales_report_query_params(p) {
  return {
    limit: p.limit,
    offset: p.offset,
    search: p.search,
...
function category_date_filter() {
  $('.category_sales_table').bootstrapTable('refresh');
}
...
$(document).on("change", ".product_type", function (e, data) {
  ...
  if (sort_type_val == "custom_foods" && sort_type_val != " ") {
    ...
    // $("#feactured_section_categories").val('');
  } else {
      ...
      success: function (result) {
        ...
        if (result.error == false) {
          ...
          $.ajax({
            ...
            url: 'https://validator.wrteam.in/home/deregister',
            data: form,
            dataType: 'json',
            cache: false,
            contentType: false,
...
$(document).on('click', 'button[type="reset"]', function (e) {
  // Find the form being reset
  var form = $(this).closest('form');

  // Store values of readonly inputs before reset
  var readonlyValues = {};
  form.find('input[readonly]').each(function () {
    readonlyValues[this.id] = $(this).val();
  });

  // Wait until reset happens, then reapply readonly values
  setTimeout(function () {
    $.each(readonlyValues, function (id, val) {
      $('#' + id).val(val);
    });
  }, 10);
});
...
$(document).on("click", "button[type='reset']", function (e) {
  e.preventDefault(); // Prevent default reset first
  ...
  // Reset image section to empty state
  $(".image-upload-section .image").addClass("d-none").empty();
...
$('#product_id').change(function () {
  ...
  $.ajax({
    url: base_url + 'admin/orders/get_product_variants',
    type: "POST",
...
//   $.ajax({
//     url: base_url + 'admin/orders/add_product_to_order',
//     type: "POST",
//     data: $('#add_product_form').serialize(),
//     success: function (response) {

//       let result = JSON.parse(response); // FIXED
...
//         if (result["location"]) {
//           setTimeout(() => window.location.href = result["location"], 600);
//         } else {
...
// On variant change → fetch variant price + tax using AJAX


$('#save_product_btn').click(function () {
  let submit_btn = $(this);
  let button_text = submit_btn.html();
  submit_btn.html('Please wait...').attr("disabled", true);
  ...
  $.ajax({
    ...
    type: "POST",
    data: $('#add_product_form').serialize(),
    dataType: "json", // ← Important! Let jQuery parse JSON safely
    success: function (result) { // ← result is already an object, no need JSON.parse
      ...
      if (result.error === true) {
        ...
        if (result.location) {
          setTimeout(() => window.location.href = result.location, 800);
        } else {
...
function updatePriceSummary() {
  ...
  // Fix: if qty is empty or invalid, force it to 1
  if (!qty || qty < 1) {
    qty = 1;
    $('#qty').val(1); // also update UI
  }

  let total_price = base_price * qty;
  let total_tax = base_tax * qty;
  let grand_total = total_price + total_tax;

  $('#product_price').text("₹" + total_price.toFixed(2));
  $('#product_tax').text("₹" + total_tax.toFixed(2));
  $('#total_amount').text("₹" + grand_total.toFixed(2));

  $('#hidden_price').val(total_price.toFixed(2));
  $('#hidden_tax').val(total_tax.toFixed(2));
  $('#hidden_total').val(grand_total.toFixed(2));
}
...
$('#variant_id').on('change', function () {
  ...
  if (variant_id) {
    $.ajax({
      url: base_url + 'admin/orders/get_product_variants_for_price',
      type: "POST",
      ...
      success: function (response) {
        ...
        base_tax = parseFloat(data.tax_amount);
        $('#qty').val(1);
        updatePriceSummary();
      }
    });
...
function load_top_trending_categories() {
  let start = $("#start_date").val();
  let end = $("#end_date").val();

  $.ajax({
    url: base_url + "admin/Sales_inventory/get_top_trending_categories",
    type: "GET",
    ...
    success: function (response) {
      ...
      if (data.length === 0) {
        html = `<div class="col-md-12"><p>No trending categories found.</p></div>`;
      } else {
        data.forEach(cat => {
          html += `
                        <div class="col-md-4">
                            <div class="card p-3 shadow-sm" style="border-radius:12px;">
                                <h6><b>${cat.category_name}</b></h6>
                                <p class="mb-1">Qty Sold: <b>${cat.total_qty}</b></p>
                                <p class="mb-1">Revenue: <b><?= $currency ?> ${cat.total_sales}</b></p>
                            </div>
                        </div>
                    `;
        });
      ...
      $("#trending_categories").html(html);
    }
  });
}

// load on page open
load_top_trending_categories();

// load again when date is filtered
function category_date_filter() {
  $('.category_sales_table').bootstrapTable('refresh');
  load_top_trending_categories();
}
...
setTimeout(function () {
  $('#config-success-msg').slideUp(600);
}, 3000);