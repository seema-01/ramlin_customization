...
$("#clear_user_search").on("click", function () {
  // Clear the selected value from Select2 properly
  $(".select_user").val(null).trigger("change");

  // Optionally clear the underlying hidden input if used
  $("#select_user_id").val('');
});
...
// also trigger when clear (X) button is clicked
$("#search_products").on("search", function () {
  var search = $(this).val().trim();
  get_products("", 25, 0, search);
});
