...
function update_stock($product_variant_ids, $qtns, $type = '')
{
    ...
    if (!empty($product_variant_ids)) {
        $res = $t->db->select('p.*, pv.*, p.id as p_id, pv.id as pv_id, p.stock as p_stock, pv.stock as pv_stock')
...
function validate_order_status($order_ids, $status, $table = 'order_items', $user_id = null)
{
    ...
    if (in_array(strtolower(trim($status)), $check_status)) {
        ...
        if ($canceling_delivered_item == true) {
            ...
            $response['error'] = true;
            $response['message'] = "You cannot cancel delivered or returned order.";
            $response['data'] = array();
            return $response;
        }
...
function process_refund($id, $status, $type = 'orders')
{
    ...
    if ($payment_method != 'cod' || $payment_method != 'COD') {
        ...
        if ($is_delivery_charge_returnable == 1) {
            ...
            $returnable_amount = $order_details[0]['total'] + $order_details[0]['tax_amount'] + $order_details[0]['delivery_tip'] - $promo_discount;
        }
        $fcmMsg = array(
            'title' => "Amount Credited To Wallet",
...
function get_delivery_charge($address_id, $user_id, $branch_id)
{
    ...
    if (is_exist(['id' => $address_id], 'addresses')) {
        ...
        // get restro id and coordinates from cart data
        // $t->db->select('u.latitude,u.longitude,p.partner_id')->join('product_variants pv', 'pv.id=c.product_variant_id')->join('products p', "pv.product_id=p.id")->join("users u", "p.partner_id=u.id")->join("users uc", "c.user_id=uc.id");
        // $t->db->where('c.user_id', $user_id);
        // $points = $t->db->from("cart c")->get()->result_array();
        // get restro id and coordinates from cart data
        $t->db->select('b.latitude,b.longitude,p.branch_id')->join('product_variants pv', 'pv.id=c.product_variant_id')->join('products p', "pv.product_id=p.id")->join("branch b", "p.branch_id=b.id")->join("users uc", "c.user_id=uc.id");
        ...
        // }
            ...
            /* find distnce with google API */
            $array_result = find_google_map_distance($city_id[0]['latitude'], $city_id[0]['longitude'], $points[0]['latitude'],  $points[0]['longitude']);
            $result = isset($array_result) ? json_decode($array_result, true) : "";

            if ($result[0]['error']['code'] == "400" && $result[0]['error']['details'][0]['reason'] == "API_KEY_INVALID") {
                $response['error'] = true;
                $response['message'] = 'The provided API key is invalid.';
                $response['charge'] = "0";
                $response['distance'] = "0";
                ...
                if ($charge_method == "range_wise_charges") {
                    if (isset($result) && $result[0]['condition'] == "ROUTE_NOT_FOUND") {
                        $response['error'] = true;
                        $response['message'] = 'Route not found between restaurant and user address.';
                        $response['charge'] = "0";
                        $response['distance'] = "0";
                        $response['duration'] = "0";
                        return $response;
                    } elseif (isset($result) && $result[0]['condition'] == "ROUTE_EXISTS") {
                        $ranges = json_decode($get_methods[0]['range_wise_charges'], true);
                        $distance = round($distance);
                        foreach ($ranges as $range) {
                            if ($distance >= $range['from_range'] && $distance <= $range['to_range']) {
                ...
                $response['error'] = false;
                $response['message'] = 'Data fetched successfully.';
                $response['charge'] = isset($charge) && !empty($charge) ? $charge : "0";
                $response['distance'] = isset($distance) && !empty($distance) ? $distance . "Km" : "0";
                $response['duration'] = isset($time) && !empty($time) ? $time : "0";
                return $response;
            }
        }
    } else {
...
function sendNotificationToPlatform($platform, $chunk, $url, $headers, $customBodyFields, $title, $message)
{
        ...
        $data = [
            "message" => [
                ...
                "notification" => [
                    "title" => (string)$customBodyFields['title'],
                    "body" => (string)$customBodyFields['body'],
                    "image" => isset($customBodyFields['image']) ? $customBodyFields['image'] : '',
                ],
                "data" => $customBodyFields
            ]
        ];

        // Platform-specific optional settings
        if ($platform == 'android') {
            $data["message"]["android"] = [
                "priority" => "high",
                "notification" => [
            ...
            $data["message"]["webpush"] = [
                "notification" => [
                    "title" => $customBodyFields['title'],
                    "body" => $customBodyFields['body'],
                    "icon" => isset($customBodyFields['image']) ? $customBodyFields['image'] : '',
                ]
            ];
        }
    ...
    foreach ($curl_handles as $ch) {
        $result = curl_multi_getcontent($ch);
        // log_message('error', 'Flutterwave notification result --> ' . var_export($result, true));
        if ($result === false) {
            error_log('Curl failed: ' . curl_error($ch));
        }
        curl_multi_remove_handle($mh, $ch);
...
if (!function_exists('display_breadcrumbs')) {
    function display_breadcrumbs()
    {
        ...
        // Detect current panel (admin, rider, etc.)
        $panel = $CI->uri->segment(1);

        // Determine home label and URL
        if ($panel == 'admin') {
            $home_url = base_url('admin/home');
            $home_label = 'Home';
        } elseif ($panel == 'rider') {
            $home_url = base_url('rider/home');
            $home_label = 'Home';
        } else {
            $home_url = base_url();
            $home_label = 'Home';
        ...
        $breadcrumbs .= '<li class="breadcrumb-item"><a class="text text-info" href="' . $home_url . '">' . $home_label . '</a></li>';

        // Build dynamic parts from URI
        $path = $panel . '/';
        $total_segments = count($uri);

        foreach ($uri as $index => $segment) {
            // Skip first segment (panel)
            if ($index == 1) continue;
            ...
            if ($index == $total_segments) {
                ...
                // Linked items
                $breadcrumbs .= '<li class="breadcrumb-item"><a class="text text-info" href="' . base_url($path) . '">' . $name . '</a></li>';
            }
        }
