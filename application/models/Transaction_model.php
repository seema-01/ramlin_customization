...
class Transaction_model extends CI_Model
{
    ...
    function get_transactions_list($user_id = '', $group_id = 2)
    {
        ...
        if (isset($_GET['search']) and $_GET['search'] != '') {
            $search = $_GET['search'];
            $multipleWhere = ['`transactions.id`' => $search, '`transactions.amount`' => $search, '`transactions.date_created`' => $search, 'users.username' => $search, 'users.mobile' => $search, 'users.email' => $search, 'transactions.type' => $search, 'transactions.status' => $search, 'transactions.txn_id' => $search, 'transactions.order_id' => $search];
        }
        ...
        foreach ($txn_search_res as $row) {
            ...
            if (isset($row['status']) && !empty($row['status']) && !is_null($row['status']) && $row['status'] == 'success' || $row['status'] == 'Success') {
            ...
            } elseif (isset($row['status']) && !empty($row['status']) && !is_null($row['status']) && $row['status'] == 'Pending') {
                $tempRow['status'] = '<span class="badge badge-warning">Pending</span>';
            } elseif (isset($row['status']) && !empty($row['status']) && !is_null($row['status']) && $row['status'] == 'awaiting') {
                $tempRow['status'] = '<span class="badge badge-dark">Awaiting</span>';
            } elseif (isset($row['status']) && !empty($row['status']) && !is_null($row['status']) && $row['status'] == 'received') {
                $tempRow['status'] = '<span class="badge badge-primary">Received</span>';
            } elseif (isset($row['status']) && !empty($row['status']) && !is_null($row['status']) && $row['status'] == 'COMPLETED') {
                $tempRow['status'] = '<span class="badge badge-success">Success</span>';
            }
            else {
    ...
    function get_wallet_transactions_list($user_id = '', $group_id = 2)
    {
        $offset = 0;
        $limit = 10;
        $sort = 'id';
        ...
        $where = [];
        if (isset($_GET['transaction_type']))
            $where = ['transactions.transaction_type' => 'wallet'];
