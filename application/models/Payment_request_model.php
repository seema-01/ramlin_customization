...
class Payment_request_model extends CI_Model
{
    ...
    function get_payment_request_list($user_id = NULL)
    {
        ...
        if (isset($_GET['search']) && $_GET['search'] != '') {
            $search = $_GET['search'];
            $multipleWhere = ['pr.`id`' => $search, 'u.`username`' => $search, 'u.`email`' => $search, 'u.`mobile`' => $search, 'pr.`payment_address`' => $search, 'pr.`remarks`' => $search];
        }
        ...
        foreach ($offer_search_res as $row) {
            ...
            $tempRow['amount_requested'] = $currency . $row['amount_requested'];
