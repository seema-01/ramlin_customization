...
class Customer_model extends CI_Model
{
    ...
    public function get_customer_list($wallet_transaction = false)
    {
        ...
        foreach ($cat_search_res as $row) {
            ...
            if ($row['active'] == '1') {
                $operate .= '<a class="btn btn-success btn-xs update_active_status mr-1 mb-1" data-table="users" title="Deactivate" href="javascript:void(0)" data-id="' . $row['id'] . '" data-status="' . $row['active'] . '" ><i class="fa fa-toggle-on"></i></a>';
            } else {
                $operate .= '<a class="btn btn-secondary mr-1 mb-1 btn-xs update_active_status" data-table="users" href="javascript:void(0)" title="Active" data-id="' . $row['id'] . '" data-status="' . $row['active'] . '" ><i class="fa fa-toggle-off"></i></a>';
            }
            ...
            $tempRow['balance'] = $currency . number_format($row['balance'], 2);
    ...
    {
        return $this->db->select('balance')->from('users')->where('id', $user_id)->get()->row('balance');
    }

    public function deduct_wallet($user_id, $amount)
    {
        $this->db->set('balance', "balance - $amount", FALSE)
            ->where('id', $user_id)
            ->update('users');
