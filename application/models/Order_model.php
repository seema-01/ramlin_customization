...
class Order_model extends CI_Model
{
    ...
    public function get_orders_list(
        ...
        if ($is_pending == true && !empty($rider_city_id)) {
            ...
            $rider_city_ids = explode(',', $rider_city_id);
            $count_res->where_in('po.city_id', $rider_city_ids);

            // Only pending stages
            $count_res->where_in('o.active_status', ['confirmed', 'preparing']);

            // Exclude orders rejected by this rider
            if (!empty($rider_id)) {
                $count_res->where("(FIND_IN_SET(" . (int)$rider_id . ", po.rejected_riders) = 0 
                            OR po.rejected_riders IS NULL)", NULL, FALSE);
            }
        }
        ...
        // }
        // Apply rider_id filter ONLY when NOT fetching pending orders.
        // For pending orders we must NOT force rider_id (pending = unassigned to rider)
        if (!empty($rider_id) && $is_pending == false) {
            $count_res->where("o.rider_id", (int)$rider_id);
        }
        ...
        if (isset($_GET['is_self_pick_up'])) {
            $count_res->where('is_self_pick_up', (int)$_GET['is_self_pick_up']);
        }
        ...
        if ($is_pending == true && !empty($rider_city_id)) {
            ...
            // Multiple cities
            $rider_city_ids = explode(',', $rider_city_id);
            $search_res->where_in('po.city_id', $rider_city_ids);
            ...
            if (!empty($rider_id)) {
                $search_res->where("(FIND_IN_SET(" . (int)$rider_id . ", po.rejected_riders) = 0
                            OR po.rejected_riders IS NULL)", NULL, FALSE);
            }
        }
        ...
        // Apply rider_id filter ONLY when NOT fetching pending orders
        if (!empty($rider_id) && $is_pending == false) {
            $search_res->where("o.rider_id", (int)$rider_id);
        }
        ...
        if (isset($_GET['is_self_pick_up']) && $_GET['is_self_pick_up'] != '') {
            $search_res->where('is_self_pick_up', (int)$_GET['is_self_pick_up']);
        }
    ...
    // modify order modal 
    public function update_totals_after_add($order_id, $amount)
    {
        $this->db->set('total', "total + $amount", FALSE);
        $this->db->set('final_total', "final_total + $amount", FALSE);
        $this->db->set('total_payable', "total_payable + $amount", FALSE);
    ...
    public function update_extra_payment($order_id, $amount)
    {
        $this->db->set('extra_payment_to_be_collect', "extra_payment_to_be_collect + $amount", FALSE);
        $this->db->where('id', $order_id);
        $this->db->update('orders');
    }


    public function insert_new_order_item($order_id, $product_id, $qty, $price, $sub_total, $product_variant_id)
    {
        $data = [
            'order_id'           => $order_id,
