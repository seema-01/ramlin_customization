...
class Address_model extends CI_Model
{
    ...
    public function get_address_list($user_id = '')
    {
        ...
        if (isset($_GET['search']) and $_GET['search'] != '') {
            $search = $_GET['search'];
            $multipleWhere = ['addr.name' => $search, 'addr.address' => $search, 'addr.mobile' => $search, 'c.name' => $search, 'state' => $search, 'country' => $search, 'addr.pincode' => $search];
        }

        $count_res = $this->db->select(' COUNT(addr.id) as `total` ,addr.*,c.name as city,u.username as username')->join('cities c', 'addr.city_id=c.id', 'left')->join('users u', 'addr.user_id=u.id', 'left');
        ...
        $search_res = $this->db->select('addr.*,c.name as city,u.username as username')->join('cities c', 'addr.city_id=c.id', 'left')->join('users u', 'addr.user_id=u.id', 'left');
        ...
        foreach ($address_search_res as $row) {
            ...
            $tempRow['id'] = $row['id'];
            $tempRow['name'] = isset($row['name']) && !empty($row['name']) ? stripslashes($row['name']) : $row['username'];
            $tempRow['type'] = $row['type'];
            ...
            $tempRow['address'] = stripslashes($row['address']);
            $tempRow['landmark'] = stripslashes($row['landmark']);
            $tempRow['area'] = stripslashes($row['area']);
            $tempRow['city'] = stripslashes($row['city']);
            $tempRow['city_id'] = $row['city_id'];
            $tempRow['state'] = stripslashes($row['state']);
            $tempRow['pincode'] = $row['pincode'];
            $tempRow['country'] = stripslashes($row['country']);
