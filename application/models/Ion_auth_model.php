...
class Ion_auth_model extends CI_Model
{
        ...
        foreach ($group_ids as $group_id) {
            ...
            if (
                ...
                if (isset($this->_cache_groups[$group_id])) {
                    ...
                    $group_name = isset($group[0]->name) ? $group[0]->name : "";
    ...
    public function set_session($user)
    {
        ...
        $user_detail = fetch_details(['user_id' => $user->id], 'user_permissions', 'branch_ids,role');

            if (isset($user_detail[0]['role']) && isset($user_detail[0]['branch_ids']) &&$user_detail[0]['role'] == "0" || empty($user_detail[0]['branch_ids'])) {
    
                $branch_id = $this->get_default_branch(); // Get the default branch ID
            } else {
             
                $branches = isset($user_detail[0]['branch_ids']) ? explode(",", $user_detail[0]['branch_ids']) : [];
