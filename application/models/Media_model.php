    ...
    public function fetch_media()
    {
        if (($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) || ($this->ion_auth->logged_in() && $this->ion_auth->is_partner() && ($this->ion_auth->partner_status() == 1 || $this->ion_auth->partner_status() == 0))) {
            ...
            foreach ($city_search_res as $row) {
                ...
                $tempRow['image'] = '<div class="image-upload-div text-center" style="width:200px; height:150px; margin:auto; overflow:hidden;">
    <span class="path d-none">' . base_url() . $row['sub_directory'] . $row['name'] . '</span>
    <span class="relative-path d-none">' . $row['sub_directory'] . $row['name'] . '</span>
    <a href="' . $row['image'] . '" data-toggle="lightbox" data-gallery="gallery">
        <img src="' . $row['image'] . '" style="width:100%; height:100%; object-fit:contain;">
    </a>
</div>';
