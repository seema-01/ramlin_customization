    ...
    public function add_product()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if(isset($_POST['calories']) && $_POST['calories'] < 0){
                ...
                $this->response['csrfName'] = $this->security->get_csrf_token_name();
                $this->response['csrfHash'] = $this->security->get_csrf_hash();
                $this->response['message'] = "Calories can't be negative";
                print_r(json_encode($this->response));
                return false;
            }
            if(isset($_POST['total_allowed_quantity']) && $_POST['total_allowed_quantity'] < 0){
                $this->response['error'] = true;
                $this->response['csrfName'] = $this->security->get_csrf_token_name();
                $this->response['csrfHash'] = $this->security->get_csrf_hash();
                $this->response['message'] = "Total Allowed Quantity can't be negative";
                print_r(json_encode($this->response));
                return false;
            }
            if(isset($_POST['minimum_order_quantity']) && $_POST['minimum_order_quantity'] < 0){
                $this->response['error'] = true;
                $this->response['csrfName'] = $this->security->get_csrf_token_name();
                $this->response['csrfHash'] = $this->security->get_csrf_hash();
                $this->response['message'] = "Minimum Order Quantity can't be negative";
                print_r(json_encode($this->response));
                return false;
            }
            ...
            if (isset($_POST['product_type']) && $_POST['product_type'] == 'simple_product') {
                ...
                $this->form_validation->set_rules('simple_special_price', 'Special Price', 'trim|numeric|less_than_equal_to[' . $this->input->post('simple_price') . ']|xss_clean|greater_than[0]');
                    ...
                    while (($row = fgetcsv($handle, 10000, ",")) != FALSE) //get row values
                    {
                        if ($temp != 0) {
                            ...
                            if (isset($row[6]) && $row[6] == '') {
                                if($row[6] < 0){
                                    $this->response['error'] = true;
                                    $this->response['message'] = 'Calories cant be negative at row ' . $temp;
                                    $this->response['csrfName'] = $this->security->get_csrf_token_name();
                                    $this->response['csrfHash'] = $this->security->get_csrf_hash();
                                    print_r(json_encode($this->response));
                                    return false;
                            ...
                            if (empty($row[19])) {
                                if($row[19] < 0){
                                    $this->response['error'] = true;
                                    $this->response['message'] = 'Branch Id cant be negative at row ' . $temp;
                                    $this->response['csrfName'] = $this->security->get_csrf_token_name();
                                    $this->response['csrfHash'] = $this->security->get_csrf_hash();
                                    print_r(json_encode($this->response));
                                    return false;
