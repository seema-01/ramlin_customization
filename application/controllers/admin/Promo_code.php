    ...
    public function add_promo_code()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                $no_of_repeat_usage = $_POST['no_of_repeat_usage'];
                ...
                if ($_POST['discount_type'] == 'percentage') {
                    ...
                    if ($discount >= $minimum_order_amount && $discount <= 0) {
                        $response["error"] = true;
                        $response['csrfName'] = $this->security->get_csrf_token_name();
                        $response['csrfHash'] = $this->security->get_csrf_hash();
                        $response["message"] = "Discount can not be grater then or equal to the Minimum order amount and Discount can not be less then or equal to zero!";
                        $response["data"] = array();
                        echo json_encode($response);
                        return false;
                    }
                ...
                if ($max_discount_amount <= 0) {
                        ...
                        $response['csrfName'] = $this->security->get_csrf_token_name();
                        $response['csrfHash'] = $this->security->get_csrf_hash();
                        $response["message"] = "Maximum discount amount is invalid !";
                        $response["data"] = array();
                        echo json_encode($response);
                        return false;
                    }
                if ($no_of_repeat_usage <= 0) {
                        $response["error"] = true;
                        $response['csrfName'] = $this->security->get_csrf_token_name();
                        $response['csrfHash'] = $this->security->get_csrf_hash();
                        $response["message"] = "No. of repeat usage is invalid !";
                        $response["data"] = array();
                        echo json_encode($response);
                        return false;
                    }
