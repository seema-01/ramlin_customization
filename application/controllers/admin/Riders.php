        ...
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (delete_details(['id' => $_GET['id']], 'users') == TRUE) {
                if (delete_details(['user_id' => $_GET['id']], 'users_groups') == TRUE) {
                    if (delete_details(['rider_id ' => $_GET['id']], 'fund_transfers') == TRUE) {
        ...
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                if (isset($_POST['commission']) && $_POST['commission'] <= 0) {
                    $response["error"]   = true;
                    $response["message"] = "Commission should be greater than zero";
                    $response['csrfName'] = $this->security->get_csrf_token_name();
                    $response['csrfHash'] = $this->security->get_csrf_hash();
                    $response["data"] = array();
                    echo json_encode($response);
                ...
                if (isset($_POST['edit_rider']) || isset($_POST['edit_rider_request'])) {
                    ...
                    if ($_POST['commission_method'] == 'percentage_on_delivery_charges') {
                        if (isset($_POST['percentage']) && !empty($_POST['percentage'])) {
                            ...
                            if (isset($_POST['max_commission']) && $_POST['max_commission'] <= 0) {
                                $response["error"]   = true;
                                $response["message"] = "Max Commission should be greater than zero";
                                $response['csrfName'] = $this->security->get_csrf_token_name();
                                $response['csrfHash'] = $this->security->get_csrf_hash();
                                $response["data"] = array();
                                echo json_encode($response);
