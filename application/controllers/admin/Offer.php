    ...
    public  function index()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
              if(isset($_GET['edit_id']) && !empty($_GET['edit_id'])){
                $offer_data = fetch_details(['id' => $_GET['edit_id']],'offers','*');
                if($offer_data[0]['branch_id'] !== $_SESSION['branch_id']){
                    redirect('admin/offer/manage-offer/', 'refresh');
                }
            }
    ...
    public function add_offer()
    {
        ...
        if (!$this->form_validation->run()) {
            ...
            $this->response['messages'] = array(
                    'offer_type' => form_error('offer_type'),
                    'image' => form_error('image'),
                    'banner' => form_error('banner'),
             ...
             $startDate = $_POST["start_date"];
              $endDate = $_POST["end_date"];
                ...
                if ($endDateTime <= $startDateTime) {
                    ...
                    $this->response['location'] = base_url('admin/offer/manage-offer');
