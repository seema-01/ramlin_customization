...
class Tag extends CI_Controller
{
    ...
    public function add_tags()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                $this->response['location'] = base_url('admin/tag/manage_tag');
                print_r(json_encode($this->response));
            }
        } else {
            redirect('admin/login', 'refresh');
    ...
    public function add_tags_from_product()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
    ...
    public function delete_tag()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (!has_permissions('delete', 'tags')) {
                $this->session->set_flashdata('authorize_flag', PERMISSION_ERROR_MSG);
                $this->response['error'] = true;
                $this->response['csrfName'] = $this->security->get_csrf_token_name();
                $this->response['csrfHash'] = $this->security->get_csrf_hash();
            ...
            if (is_exist(['tag_id' => $tag_id], 'product_tags')) {
                ...
                $this->response['error'] = true;
                $this->response['message'] = 'Tag is used in product';
            } else {
