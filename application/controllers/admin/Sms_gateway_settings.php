...
class SMS_gateway_settings extends CI_Controller
{
    ...
    public function add_sms_data()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            $this->form_validation->set_rules('base_url', 'Base URL', 'trim|required|xss_clean|valid_url');
            $this->form_validation->set_rules('sms_gateway_method', 'Method ', 'trim|required|xss_clean');
            $this->form_validation->set_rules('var_header_key', 'Header', 'trim|xss_clean');
            $this->form_validation->set_rules('var_header_value', 'Header value', 'trim|xss_clean');
