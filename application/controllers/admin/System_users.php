...
class System_users extends CI_Controller
{
    ...
    public function add_system_users()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (isset($_GET['edit_id']) && !empty($_GET['edit_id'])) {
                ...
                $user_permissions = $this->db->select('u.id,u.username,u.mobile,u.email,up.role,up.permissions,up.branch_ids')->join('users u', 'up.user_id=u.id')->where('up.id', $_GET['edit_id'])->get('user_permissions up')->result_array();

                if (!empty($user_permissions)) {
                    
                    $this->data['fetched_data'] = $user_permissions;
                    $this->data['selected_branches'] = explode(',', $user_permissions[0]['branch_ids']);
                    $this->data['branch_details'] = $this->db->get('branch')->result_array();
                }
            } else {
                $system_user_permissions = $this->db->select('u.id,u.username,u.mobile,u.email,up.role,up.permissions')->join('users u', 'up.user_id=u.id')->where('up.user_id', $_SESSION['user_id'])->get('user_permissions up')->result_array();
              
                if (!empty($system_user_permissions)) {
                    $this->data['system_user_permissions'] = $system_user_permissions;
                }
            }
        ...
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                if (isset($edit_id) && !empty($edit_id)) {
                    ...
                    if (is_exist(['email' => $_POST['email']], 'users', $_POST['edit_system_user'])) {
                        $this->response['error'] = true;
                    ...
                    if (is_exist(['email' => $_POST['email']], 'users')) {
                        $this->response['error'] = true;
                        $this->response['message'] = 'Email is already registered.  Please Provide Unique Email !';
                        $this->response['csrfName'] = $this->security->get_csrf_token_name();
                        $this->response['csrfHash'] = $this->security->get_csrf_hash();
                ...
                $this->response['csrfName'] = $this->security->get_csrf_token_name();
                $this->response['csrfHash'] = $this->security->get_csrf_hash();
                $this->response['location'] = base_url('admin/system-users');
    ...
    {
        $branch_id =  (isset($_GET['branch_id']) && $_GET['branch_id'] != "") ? $this->input->get('branch_id', true) : NULL;

        return $this->system_users_model->get_users_list($branch_id);
    }
}
