        ...
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            foreach ($res as $item) {
                ...
                if ($platform == '' || $platform == null) {
                    // Try to detect iOS if token doesn't start with "APA91"
                    if (isset($item['fcm_id']) && strpos($item['fcm_id'], 'APA91') !== 0) {
                        $platform = 'ios';
                    } else {
                        $platform = 'android';
                    }
                ...
                // Push FCM IDs into appropriate arrays
                if (!empty($item['fcm_id'])) {
                    $groupedByPlatform[$platform][] = $item['fcm_id'];
                }

                // Handle web tokens safely
                if (!empty($item['web_fcm_id'])) {
                    $groupedByPlatform['web'][] = $item['web_fcm_id'];
                }
            }
