...
class Orders extends CI_Controller
{
    ...
    public function view_pending_orders()
    {
        ...
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_rider()) {
            ...
            return $this->Order_model->get_orders_list($deliveryBoyId, true, $city_id[0]['serviceable_city']);
        } else {
            redirect('rider/login', 'refresh');
        }
    }
    ...
    public function update_order_request()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_rider()) {
            ...
            if ($accept_order == "1") {
                ...
                $fetch_rejected_riders = fetch_details(['order_id' => $order_id], "pending_orders", "rejected_riders");

                if (!empty($fetch_rejected_riders)) {
                    $existing_riders = $fetch_rejected_riders[0]['rejected_riders'];

                    // If already some riders have rejected
                    if (!empty($existing_riders)) {
                        // Convert to array
                        $rider_ids = explode(',', $existing_riders);

                        // Add new rider only if not already in list
                        if (!in_array($rider_id, $rider_ids)) {
                            $rider_ids[] = $rider_id;
                        }

                        // Convert back to comma-separated string
                        $updated_riders = implode(',', $rider_ids);
                    } else {
                        // If no one has rejected yet
                        $updated_riders = $rider_id;
                    }

                    // Update the table with new list
                    if (update_details(['rejected_riders' => $updated_riders], ['order_id' => $order_id], "pending_orders")) {
                        $this->response['error'] = false;
                        $this->response['message'] = "Order rejected.";
                    } else {
                        $this->response['error'] = true;
