    ...
    public function get_rider_details()
    {
        ...
        */
        ...
        if (isset($data) && !empty($data)) {
            ...
            $awaiting = orders_count("awaiting", "", $_POST['id']);
            $pending = orders_count("pending", "", $_POST['id']);
            $confirmed = orders_count("confirmed", "", $_POST['id']);
            $preparing = orders_count("preparing", "", $_POST['id']);
            $ready_for_pickup = orders_count("ready_for_pickup", "", $_POST['id']);
            $out_for_delivery = orders_count("out_for_delivery", "", $_POST['id']);
            $draft = orders_count("draft", "", $_POST['id']);
            $data['pending_deivery'] = strval(intval($awaiting) + intval($pending) + intval($confirmed) + intval($preparing) + intval($ready_for_pickup) + intval($out_for_delivery) + intval($draft));
    ...
    */
    ...
    public function update_user()
    {
        ...
        */
        ...
        if (!$this->form_validation->run()) {
            ...
            // print_r($_POST)
            if (isset($_POST['accept_orders']) && $_POST['accept_orders'] !== NULL && $_POST['accept_orders'] !== "") {
                if (defined('ALLOW_MODIFICATION') && ALLOW_MODIFICATION == 0) {
                    $this->response['error'] = true;
                    $this->response['message'] = DEMO_VERSION_MSG;
                    echo json_encode($this->response);
    ...
    public function update_order_status()
    {
        ...
        /* 
            ...
            reason:value        {required when status is cancelled}
        */
    ...
    public function get_all_details()
    {
        ...
        */
        ...
        if (!$this->form_validation->run()) {
            ...
            $awaiting = orders_count("awaiting", "", $rider_id);
            $pending = orders_count("pending", "", $rider_id);
            $confirmed = orders_count("confirmed", "", $rider_id);
            $preparing = orders_count("preparing", "", $rider_id);
            $ready_for_pickup = orders_count("ready_for_pickup", "", $rider_id);
            $out_for_delivery = orders_count("out_for_delivery", "", $rider_id);
            $draft = orders_count("draft", "", $rider_id);
            $data['pending_deivery'] = strval(intval($awaiting) + intval($pending) + intval($confirmed) + intval($preparing) + intval($ready_for_pickup) + intval($out_for_delivery) + intval($draft));
