        ...
        if ($this->form_validation->run() === TRUE) {
            ...
            if (!empty($res)) {
                if ($this->ion_auth_model->in_group('admin', $res[0]['id'])) {
                    ...
                    $get_user = fetch_details(['mobile' => $_POST['identity']], 'users', 'id');
                    if (!empty($get_user)) {
                        $has_branch = fetch_details(['user_id' => $get_user[0]['id']], 'user_permissions', 'role,branch_ids');
                        if ($has_branch[0]['role'] == 0) {
                        
                        } else {
                            if (empty($has_branch[0]['branch_ids'])) {
                                $response['error'] = true;
                                $response['csrfName'] = $this->security->get_csrf_token_name();
                                $response['csrfHash'] = $this->security->get_csrf_hash();
                                $response['message'] = 'There is no branch assigned to you ! Please contact Super Admin';
