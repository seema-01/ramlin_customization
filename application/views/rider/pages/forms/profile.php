                    ...
                    <div class="card card-info">
                        ...
                        <form class="form-submit-event" action="<?= base_url('rider/login/update_user') ?>" method="POST">
                            <div class="card-body">
                                <div class="row">
                                    <div class="col">
                                        ...
                                        <?php if (!empty($users->commission_method) && $users->commission_method == "fixed_commission_per_order") { ?>
                                            <br><label class="h5">Commission: <span class="text text-info"><?= $users->commission . "(" . $curreny . ")" ?> </span></label>
                                        <?php } ?>
                                        <?php if (!empty($users->commission_method) && $users->commission_method == "percentage_on_delivery_charges") { ?>
                                            <br><label class="h5">Commission: <span class="text text-info"><?= $users->commission . "(%)" ?> </span></label>
                                            <br><label class="h5">Max Commission: <span class="text text-info"><?=  $curreny . $users->max_commission?> </span></label>
                                ...
                                <div class="row">
                                    <div class="col">
                                        <label class="h5">Servicing City: <span class="text text-info"><?= (isset($users->serviceable_city_names) && !empty($users->serviceable_city_names)) ? $users->serviceable_city_names : ""; ?> </span></label>
                                        <p class="text text-info">If you want to change servicing city then please contact super admin.</p>
                                    </div>
                                </div>
                                <hr>
                                ...
                                <div class="form-group row">
                                    ...
                                    <div id="status" class="btn-group col-sm-8">
                                        <label class="btn btn-default" data-toggle-class="btn-default" data-toggle-passive-class="btn-default">
                                            <input type="radio" name="status" value="0" <?= (isset($users->accept_orders) && $users->accept_orders == '0') ? 'Checked' : '' ?>> No
                                        </label>
                                        <label class="btn btn-primary" data-toggle-class="btn-primary" data-toggle-passive-class="btn-default">
                                            <input type="radio" name="status" value="1" <?= (isset($users->accept_orders) && $users->accept_orders == '1') ? 'Checked' : '' ?>> Yes
                                        </label>
                                    </div>
                                </div>
